...
def create_example_input(batch_size, model_type,device, shuffle=False ):
    if model_type == VANILLA_GAN or model_type == BASIC_VAE:
        ...
        im, masks = iter(DataLoader(IMAGE_DIR, MASK_DIR, batch_size, shuffle=shuffle).get_data_loader()).next()
